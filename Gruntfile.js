...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        compress: {
            ...
            finalWindowsApp: {
                options: {
                    archive: '<%= config.dist %>/iCheckIPA.zip'
                },
                files: [{
                    expand: true,
                    cwd: '<%= config.tmp %>',
        ...
        rename: {
            app: {
                files: [{
                    ...
                    dest: '<%= config.dist %>/iCheckIPA.app'
                }]
            },
            zipToApp: {
                files: [{
    ...
    grunt.registerTask('chmod', 'Add lost Permissions.', function() {
        var fs = require('fs');
        fs.chmodSync('dist/iCheckIPA.app/Contents/Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
        fs.chmodSync('dist/iCheckIPA.app/Contents/Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
        fs.chmodSync('dist/iCheckIPA.app/Contents/Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
        fs.chmodSync('dist/iCheckIPA.app/Contents/MacOS/node-webkit', '555');
    });
    ...
    grunt.registerTask('createWindowsApp', 'Create windows distribution.', function() {
        ...
        var childProcess = require('child_process');
        var exec = childProcess.exec;
        exec('copy /b tmp\\nw.exe+tmp\\app.nw tmp\\iCheckIPA.exe && del tmp\\app.nw tmp\\nw.exe && echo.>tmp\\ready', function(error, stdout, stderr) {
            console.log(stderr, stdout, error);
        });
    ...
    grunt.registerTask('setVersion', 'Set version to all needed files', function(version) {
        ...
        var infoPlistTmp = grunt.file.read(resourcesPath + '/mac-files/Info.plist.tmp', {
            encoding: 'UTF8'
        });
        ...
        grunt.file.write(resourcesPath + '/mac-files/Info.plist', infoPlist, {
            encoding: 'UTF8'
        });
    });
